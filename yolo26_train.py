...
def create_balanced_datasets(opt):
    ...
    print(f"original training set: {len(train_images)} images")
    print(f"original testing set: {len(val_images)} images")
    
    # 3. positive / negative samples split
    ...
    print(f"training set distribution: {len(train_positives)} positive samples, {len(train_negatives)} nenagtive samples")
    
    # 4. balanced 90% postive samples
    if opt.train_positive_ratio >= 1.0:
        balanced_train = train_positives
        remaining_negatives = train_negatives
    else:

        desired_train_negatives = int(len(train_positives) * (1 - opt.train_positive_ratio) / opt.train_positive_ratio)
        ...
        print(f"balanced dataset: {len(balanced_train)} images")
        print(f"  - positive samples: {len(train_positives)} ({len(train_positives)/len(balanced_train)*100:.1f}%)")
        print(f"  - : {len(selected_negatives)} ({len(selected_negatives)/len(balanced_train)*100:.1f}%)")
        print(f"  - remaining : {len(remaining_negatives)} images")
    ...
    print(f"validation set distribution: {len(val_positives)} positive samples, {len(val_negatives)} negative samples")
    print(f"validation positive sample ratio: {len(val_positives)/len(val_images)*100:.1f}%")
    ...
    if len(remaining_negatives) >= neg_samples_needed:
        ...
        print(f"warning: insufficient negative samples" )
    ...
    print(f"temp config: {balanced_config_path}")
...
def parse_opt():
    ...
    parser.add_argument('--train-positive-ratio', type=float, default=0.9, 
                       help='positive ratio (0.0-1.0)，default: 0.9')
